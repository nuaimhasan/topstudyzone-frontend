...
export default function AddSubChapter() {
  ...
  const { data } = useGetAcademySubChaptersQuery({
    ...subChapterQuery,
  });
  const subChapters = data?.data;

  const [addAcademySubChapter, { isLoading }] =
    useAddAcademySubChapterMutation();
  ...
  const handleAdd = async (e) => {
    ...
    const res = await addAcademySubChapter(info);
  ...
  return (
    <section>
      <div className="bg-base-100 rounded shadow">
        <h2 className="p-2 border-b text-lg font-medium">Add Sub Chapter</h2>
          ...
          <div className="mt-4 grid sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                required
                value={
                  subChapters?.data?.length ? subChapters?.data?.length + 1 : 1
                }
              />
            </div>
          </div>
